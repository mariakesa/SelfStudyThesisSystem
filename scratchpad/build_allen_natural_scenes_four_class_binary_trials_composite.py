#!/usr/bin/env python3
"""
Build composite Allen natural-scenes binary trial-response dataset.
...
where each value is the original binary trial response:

    X[i, k] = binary event response for neuron i on presentation/trial column k

Column order is assumed to be image-major:

    image 0 trial 0..49, image 1 trial 0..49, ..., image 117 trial 0..49

So:

    image_index = column // 50
    trial_index = column % 50
"""
...
OUTPUT_PATH = OUTPUT_DIR / "allen_natural_scenes_four_class_binary_trials_composite.npy"
...
def validate_binary_trial_matrix(matrix: np.ndarray, source: Path) -> np.ndarray:
    """
    Validate a neurons × 5900 trial-response matrix and return it unchanged.

    NaNs are allowed because they may encode missing/invalid presentations.
    Finite values are expected to be binary 0/1.
    """
    if matrix.ndim != 2:
        raise ValueError(f"Expected 2D matrix for {source.name}, got shape {matrix.shape}")
    ...
    if finite_values.size > 0:
        ...
        non_binary_values = unique_values[~np.isin(unique_values, [0, 1])]
        if non_binary_values.size > 0:
            preview = non_binary_values[:10]
            raise ValueError(
                f"Expected finite values to be binary 0/1 in {source.name}, "
                f"but found non-binary values, first few: {preview}"
            )

    return matrix


def make_trial_level_stimulus_metadata(labels: np.ndarray) -> dict:
    """
    Create stimulus/presentation metadata aligned column-by-column with X.

    X has 5900 columns, so each image label is repeated for its 50 trials.
    """
    image_index_by_column = np.repeat(np.arange(N_IMAGES, dtype=int), N_TRIALS)
    trial_index_by_column = np.tile(np.arange(N_TRIALS, dtype=int), N_IMAGES)
    label_by_column = labels[image_index_by_column]

    return {
        "column_index": np.arange(EXPECTED_COLUMNS, dtype=int),
        "image_index": image_index_by_column,
        "trial_index": trial_index_by_column,
        "label": label_by_column,
        "label_name": np.asarray(
            [LABEL_NAMES[int(x)] for x in label_by_column],
            dtype=object,
        ),
        "image_label": labels,
        "image_label_name": np.asarray(
            [LABEL_NAMES[int(x)] for x in labels],
            dtype=object,
        ),
        "label_names": LABEL_NAMES,
        "description": (
            "Stimulus metadata aligned column-by-column with X. "
            "X has total_neurons × 5900 columns. For column k in X, use "
            "stimulus_metadata['image_index'][k], "
            "stimulus_metadata['trial_index'][k], "
            "stimulus_metadata['label'][k], and "
            "stimulus_metadata['label_name'][k]. Image-level labels are also "
            "stored as stimulus_metadata['image_label'] with shape (118,)."
        ),
    }
...
def main() -> None:
    ...
    print("=" * 80)
    print("Building Allen natural-scenes four-class binary trial composite dataset")
    print("=" * 80)
    ...
    for path in files:
        ...
        try:
            matrix = np.load(path)
            matrix = validate_binary_trial_matrix(matrix, path)
            ...
            binary_trial_matrices.append(matrix)
            ...
            # For each neuron × image, this says how many non-NaN trials are present.
            ...
            session_nan_counts[sid] = int(np.isnan(matrix).sum())

            used_files.append(str(path))

            print(f"  brain area: {brain_area}")
            print(f"  neurons:    {n_neurons}")
            print(f"  output:     {matrix.shape}")
            print(f"  NaNs in binary trial matrix: {session_nan_counts[sid]}")
            print()
    ...
    stimulus_metadata = make_trial_level_stimulus_metadata(labels)
    ...
    composite = {
        ...
        "session_nan_counts": session_nan_counts,
        ...
        "constants": {
            ...
            "column_order": "image-major: image_index = column // n_trials; trial_index = column % n_trials",
        },
        "description": (
            "Composite Allen natural-scenes binary trial-response dataset. "
            "X is total_neurons × 5900. Rows are neurons concatenated across "
            "ophys sessions. Columns are individual natural-scene presentations "
            "in image-major order: image 0 trial 0..49, image 1 trial 0..49, "
            "..., image 117 trial 0..49. X[i, k] is the original binary event "
            "response for neuron i on presentation column k; trials are not "
            "averaged into probabilities. Neuron metadata is row-aligned with X. "
            "Stimulus metadata is column-aligned with X."
        ),
    }
    ...
    print(f"Total trial columns:      {X.shape[1]}")
    print(f"Images × trials:          {N_IMAGES} × {N_TRIALS}")
    ...
    print()
    print("Stimulus image-label counts:")
    for label_value, label_name in LABEL_NAMES.items():
        count = int(np.sum(labels == label_value))
        print(f"  {label_value:>2} = {label_name:<16} n={count}")
